    ...
    def run_admin(self):
        ...
        def login():
            ...
            if (self.confirm_user(username,password)):
                ...
                connection.commit()
                connection.close()
                alogin = Admin_login(self.root,self.frame,self.username,self.password,self.name)
                alogin.login()
            else:
        ...
        def back():
            response = messagebox.askyesno("Admin tasks","Do you want to Go Back?")
            if (response==True):
                self.root.destroy()
        ##############################
        ...
        login_button.grid(row=6, column=0, pady=20, columnspan=2)
        ...
        back_button.grid(row=7, column=0, pady=10, columnspan=2)
